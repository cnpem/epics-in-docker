...
Ensure that each proposed commit is an operation that applies a set of distinct
changes as a single operation (in other words, an atomic commit). The title
should follow the pattern `<scope>: <description>`. The message should properly
document why the changes should be done in the proposed way, possibly including
context information and motivations, present tradeoffs that were considered,
alternative implementations considered and any other relevant information, such
as co-authors, external references and related commits.
